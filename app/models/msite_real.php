<?php
class Msite_real extends CI_Model{
    ...
    function getSearchData($name, $menu, $order, $record_number, $record_start, $status = 2){
        $this->db->select('n.*, m.Name as MName');
        $this->db->from($this->_table." n");
        $this->db->join('site_real_menu m', 'n.Menu = m.ID');
        $where = "";
        ...
        if($name){
            if(strlen($where) > 10)
                ...
                $where = "";                                                    
            $where .= " (n.Name = '$name' OR n.ID = '$name' OR n.Name = '$name' OR n.Name like '%$name%' OR n.Name like '%$name' OR n.Name like '$name%')";
        }
        if($where)
    ...
    function countSearchData($name, $menu, $status = 2){
        ...
        if($name){
            if(strlen($where) > 10)
                ...
                $where = "";                                                    
            $where .= " (n.Name = '$name' OR n.Name like '%$name%' OR n.Name like '%$name' OR n.Name like '$name%')";
        }
        if($where)                
            $this->db->where($where, NULL, FALSE);
        $query = $this->db->get();

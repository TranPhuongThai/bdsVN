...
if($product_menu_check['Status'] == 1){
    $status['checked'] = "checked";
}
$mtit = array(
    "name"      => "mtit",
    "value"     => $product_menu_check['MTit'],
    "class"     => "text width-400 height-32 margin-0 padding-2 border-1 border-color-ccc border-style-solid",
);
$mkey = array(
    "name"      => "mkey",
    "value"     => $product_menu_check['MKey'],
    "class"     => "text width-400 height-32 margin-0 padding-2 border-1 border-color-ccc border-style-solid",
);
$mdes = array(
    "name"      => "mdes",
    "value"     => $product_menu_check['MDes'],
    "class"     => "text width-400 height-32 margin-0 padding-2 border-1 border-color-ccc border-style-solid",
);
$submit = array(
    'name'      => 'bntOk',
...
<div class="content content-menu padding-10 margin-10 background-color-f4f4f4 border-1 border-radius-8 border-color-ccc border-style-solid">
    <div class="section-left float-left">
        ...
        <?php 
            ...
            echo "<div class=\"item item-text hidden\">".form_label(lang('backend.content')." <span class=\"color-red\">*</span>").form_textarea($content)."<script type=\"text/javascript\">var fullVi = new InnovaEditor(\"fullVi\");FullEditor(fullVi, \"CONTENT\", 400);</script><div class=\"clear-both\"></div></div>";
    ...
    <div class="section-right float-right">
        <table border="0" cellpadding="0" cellspacing="0" class="table-list-item">
            ...
            <?php foreach($product_menu_list as $row){ ?>
            <tr class="row">
                <td class="row-1 text-align-center"><?php echo $row['ID'];?></td>
                <td class="row-2 text-align-center"><?php echo $row['PID'];?></td>
                <td class="row-3"><a href="<?php echo base_url()."admin/site_product_menu/edit/".$row['ID'];?>" title="<?php echo lang('backend.edit');?>" class="color-blue hover-underline"><?php echo $row['Name'];?></a><br /></td>

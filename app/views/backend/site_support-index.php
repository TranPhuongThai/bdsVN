...
<div class="content content-support padding-10 margin-10 background-color-f4f4f4 border-1 border-radius-8 border-color-ccc border-style-solid">
    ...
    <table border="0" cellpadding="0" cellspacing="0" class="table-list-item">
        <tr class="head">
            ...
            <td class="row-4"><?php echo lang('backend.yahoo');?></td>
            <td class="row-5"><?php echo lang('backend.skype');?></td>
            <td class="row-6"><?php echo lang('backend.status');?></td>
            <td class="row-7">
                <a href="<?php echo base_url()."admin/site_support/add";?>" class="bntAddBig  padding-0 margin-0 text-align-left font-weight-bold border-0 cursor-pointer hover-blue no-underline border-radius-5 hover-border-shadow"><?php echo lang('backend.add');?></a>
            </td>
        </tr>
        <?php $stt = 1; foreach($support_list as $row){ ?>
        <tr class="row">
            <td class="row-1 text-align-center"><?php echo $stt;?></td>
            <td class="row-2 text-align-center"><?php echo $row['ID'];?></td>
            <td class="row-3"><a href="<?php echo base_url()."admin/site_support/edit/".$row['ID'];?>" title="<?php echo lang('backend.edit');?>" class="color-blue hover-underline"><?php echo $row['Name'];?></a></td>
            <td class="row-4"><?php echo $row['Yahoo'];?></td>
            <td class="row-4"><?php echo $row['Skype'];?></td>
